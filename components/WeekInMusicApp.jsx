    ...
    <div className="p-0">
      ...
      <header className="mb-6 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <div className="w-20 h-20 bg-gradient-to-br from-amber-900 via-amber-700 to-amber-500 rounded-2xl flex items-center justify-center shadow-lg">
            <div className="text-white text-2xl font-bold">W/M</div>
          </div>
          <div>
        ...
        <nav className="flex items-center gap-3 text-neutral-700">
          <a className="p-2" title="Email"><Mail /></a>
          <a className="p-2" title="TikTok"><Tiktok /></a>
          <a className="p-2" title="Facebook"><Facebook /></a>
          <a className="p-2" title="Website"><Globe /></a>
          <a className="p-2" title="Instagram"><Instagram /></a>
        </nav>
      </header>
      ...
      <div className="grid grid-cols-12 gap-6">
          ...
          <div className="bg-white rounded-2xl shadow border p-4">
            ...
            <div className="space-y-3">
              ...
              <div className="flex gap-2">
                <button onClick={addEvent} className="inline-flex items-center gap-2 px-3 py-2 rounded-xl bg-black text-white hover:opacity-90"><Plus size={16}/> Add</button>
        ...
        <section className="col-span-12 lg:col-span-8">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            {filtered.map((ev) => (
              <article key={ev.id} className="bg-white rounded-2xl shadow border p-4">
                <div className="flex justify-between items-start gap-3">
                  ...
                  <div className="flex flex-col items-end gap-2">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${ev.paid ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>{ev.paid ? 'Paid' : 'Unpaid'}</span>
                    <div className="flex gap-2">
                      <button className="p-2 rounded-lg border" title="Toggle Paid" onClick={() => togglePaid(ev.id)}><Pencil size={14}/></button>
                      <button className="p-2 rounded-lg border" title="Delete" onClick={() => removeEvent(ev.id)}><Trash2 size={14}/></button>
                    </div>
                  </div>
                </div>
              </article>
